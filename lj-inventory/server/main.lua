...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			elseif name == "dryhook" then
				secondInv.name = "dryhook-"..id
				secondInv.label = other.label
				secondInv.maxweight = 10000
				secondInv.inventory = other.items
				secondInv.slots = other.slots
...
* Not Used for some Reason | Haven't found out Why...
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "dryhook" then
				-- Drying Buds
				local dryBudId = QBCore.Shared.SplitStr(toInventory, "-")[2]
				local toItemData = exports['qb-customdrugs']:GetInventoryData(dryBudId, toSlot)
				local IsItemValid = exports['qb-customdrugs']:CanItemBeDried(fromItemData.name:lower())
				local isItemDrying = exports['qb-customdrugs']:isItemAlreadyDrying(dryBudId)
				print(tostring(isitemDrying))
				if not isItemDrying then
					if IsItemValid then
						TriggerClientEvent('okokNotify:Alert', source, "Plant Drying", "You put up your plant to dry", 5000, 'success')
						RemoveItem(src, fromItemData.name, fromAmount, fromSlot)
						TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
						if toItemData then
							local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
							toAmount = tonumber(toAmount) or toItemData.amount
							if toItemData.name ~= fromItemData.name then
								exports['qb-customdrugs']:RemoveDryBud(dryBudId, fromSlot, itemInfo["name"], toAmount)
								AddItem(src, toItemData.name, toAmount, fromSlot, toItemData.info)
								TriggerEvent("qb-log:server:CreateLog", "DryingBuds", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - Hook: *" .. dryBudId .. "*")
							end
						else
							local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
							TriggerEvent("qb-log:server:CreateLog", "DryingBuds", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. dryBudId .. "*")
						end
						local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
						exports['qb-customdrugs']:AddDryBud(dryBudId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
					else
					--TriggerClientEvent('QBCore:Notify', src, "You can\'t dry this item..", 'error')
					TriggerClientEvent('okokNotify:Alert', source, "What?", "You can't dry this here...", 5000, 'error')
					end
				else
					TriggerClientEvent('okokNotify:Alert', source, "Space Occupied", "Something is on the hook already!", 5000, 'error')
				end
			else
				-- drop
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "dryhook" then
		local dryBudId = QBCore.Shared.SplitStr(fromInventory, "-")[2]
		local fromItemData = exports['qb-customdrugs']:GetInventoryData(dryBudId, fromSlot)
		fromAmount = tonumber(fromAmount) or fromItemData.amount
		if fromItemData and fromItemData.amount >= fromAmount then
			local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = GetItemBySlot(src, toSlot)
				exports['qb-customdrugs']:RemoveDryBud(dryBudId, fromSlot, itemInfo["name"], fromAmount)
				if toItemData then
					local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
					toAmount = tonumber(toAmount) or toItemData.amount
					if toItemData.name ~= fromItemData.name then
						RemoveItem(src, toItemData.name, toAmount, toSlot)
						exports['qb-customdrugs']:AddDryBud(dryBudId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						TriggerEvent("qb-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** Hook: *" .. dryBudId .. "*")
					else
						TriggerEvent("qb-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from hook: *" .. dryBudId .. "*")
					end
				else
					TriggerEvent("qb-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** hook: *" .. dryBudId .. "*")
				end
				AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info)
			else
				local toItemData = exports['qb-customdrugs']:GetInventoryData(dryBudId, toSlot)
				...
				if toItemData then
					...
					if toItemData.name ~= fromItemData.name then
						local itemInfo = QBCore.Shared.Items[toItemData.name:lower()]
						exports['qb-customdrugs']:RemoveDryBud(dryBudId, toSlot, itemInfo["name"], toAmount)
						exports['qb-customdrugs']:AddDryBud(dryBudId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
				end
				local itemInfo = QBCore.Shared.Items[fromItemData.name:lower()]
